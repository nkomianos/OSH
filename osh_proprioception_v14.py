"""
OSH Proprioceptive Training V14 — Hybrid with inverted SYNTHESIS format.

Identical to V13 EXCEPT the SYNTHESIS bridge samples are reformatted to
"No. [causal trace]" instead of "[causal trace]. So no.". Per PI directive
(2026-05-06): the V13 ordering tanked first-token Yes/No logit evals
(held-out, Anthropic 322-Q). V14 inverts the order so the answer-token
appears first, then the causal trace follows as supporting context.
...
Usage:
    python osh_proprioception_v14.py
"""
...
from v14_hybrid_curriculum import generate_v14_curriculum


V14_OUTPUT = "./osh_proprioceptive_v14"
...
def main():
    print("=" * 70)
    print("OSH PROPRIOCEPTIVE TRAINING V14 — HYBRID (INVERTED SYNTHESIS)")
    print("=" * 70)
    ...
    print("\n[5/5] Building hybrid (V14) curriculum and training...")
    full_curriculum = generate_v14_curriculum()
    ...
    pbar = tqdm.tqdm(training_data, desc="V14 hybrid")
    for sample in pbar:
        text = sample["text"]
    ...
    os.makedirs(V14_OUTPUT, exist_ok=True)
    model.save_pretrained(V14_OUTPUT)
    tokenizer.save_pretrained(V14_OUTPUT)
    print(f"\n✓ V14 saved to {V14_OUTPUT}")
    print(f"  Final avg loss: {total_loss / max(n_steps,1):.4f}")
    print(f"  Total steps:    {n_steps}")
    print(f"  Per-type avg loss:")
    for t in sorted(type_loss_sum):
        print(f"    {t:<14s} {type_loss_sum[t]/type_loss_n[t]:.4f}  (n={type_loss_n[t]})")
