...
def _repair_load_adapter_weights(corrupt_dir, reference_dir):
    ...
    Both adapters (antidote and V9) were produced by the same PEFT save_pretrained()
    call with identical LoRA configs, so they share the same tensor names, dtypes,
    shapes, and data-section byte offsets.  Only V9's 8-byte header-length prefix
    is wrong — the raw tensor data is intact and in the exact same layout.
    """
    import struct
